...
def bniio(men):
    ...
    if men.text == '/menuu':
        ...
        try:
        	bot.reply_to(men, '<b>' '' '</b>' + '\n\n' + '<b>' 'CEP</b><code> /cep < CONSULTA CEP >' '</code>' + '\n' + '<b>' 'BIN</b><code> /bin < 6 PRIMEIROS NÚMEROS DO CARTÃO >' + '</code>' '\n' + '<b>' 'CNPJ</b><code> /cnpj < PUXA DADOS DO CPNJ >' + '</code>' '\n' + '<b>' 'CPF ' '</b>''<code>' '/cpf < CONSULTA CPF SIMPLES >' '</code>' + '\n' + '<b>' 'TRABALHOS ' '</b>''<code>' '/trabalhos < CONSULTA CPF AVANÇADA >' '</code>' + '\n' + '<b>' 'TELEFONE ' '</b>' '<code>' + '/tel < PUXA DADOS DO NÚMERO >' + '</code>' + '\n' + '<b>' 'NOME ' '</b>' '<code>' + '/nome < PUXA DADOS DO NOME >' + '</code>' + '\n' + '<b>' 'VIZINHOS ' '</b>' '<code>' + '/vizinhos < PUXA DADOS DE VIZINHOS COM CPF >' + '</code>' + '\n' + '<b>' 'CEP V2 ' '</b>' '<code>' + '/cepe < CONSULTA CEP AVANÇADA >' + '</code>' + '\n' + '<b>' 'CPF V2 ' '</b>' '<code>' + '/master < CONSULTA CPF AVANÇADA >' + '</code>' + '\n' + '<b>' 'EMAIL ' '</b>' '<code>' + '/email < PUXA DADOS DO EMAIL >' + '</code>' + '\n' + '<b>' 'PARENTES ' '</b>' '<code>' + '/parentes < PUXA DADOS DOS PARENTES COM CPF >' + '</code>' + '\n' + '<b>' 'IP ' '</b>' '<code>' + '/ip < PUXA DADOS DO IP >' + '</code>' + '\n' + '<b>' 'PLACA ' '</b>' + '<code>' '/placa < PUXA DADOS DA PLACA >' '</code>' + '\n\n' + '<b>' 'CHK CC</b><code>/chkcc < VALIDAR CC > ' '</code>' '\n\n' + '<b>' 'GERAR CPF</b><code> /gencpf < GERA CPF FAKE >' '</code>' + '\n' + '<b>' 'GERAR EMAIL</b><code> /genemail < GERA EMAIL FAKE >' + '</code>' + '\n' + '<b>' 'GERAR CNPJ</b><code> /gencnpj < GERA CNPJ FAKE >' + '</code>' '\n\n' + '<b>' 'VALIDAR CPF</b><code> /validar < VALIDAR CPF >' + '</code>' + '\n\n' + '<b>' 'ID' '</b>' '<code>' + '/id [ OFF ]' + '</code>' + '\n\n' + '<b>Coded BY: @StarkVendasOFC</b>' , parse_mode='HTML')
        except:
